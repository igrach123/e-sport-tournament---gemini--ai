...
const StorageManager: React.FC<StorageManagerProps> = ({ 
  ...
  const [pendingImportData, setPendingImportData] = useState<{players: Player[], tournaments: Tournament[]} | null>(null);
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(data.players) && Array.isArray(data.tournaments)) {
            // Store data and show confirmation modal instead of window.confirm
  ...
  const executeImport = () => {
    if (pendingImportData) {
        setPlayers(pendingImportData.players);
  ...
  return (
    <>
        ...
        <ConfirmationModal
            ...
            message="ARE YOU SURE? This will permanently delete all players and tournaments. This action cannot be undone."
        ...
        <ConfirmationModal
            ...
            title="Import Data?"
            message="This will overwrite your current data with the selected file. Any unsaved changes in your current session will be lost. Are you sure you want to proceed?"
